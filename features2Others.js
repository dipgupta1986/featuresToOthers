...
commander
    ...
    .option('-o, --output-file [path]', 'send output to file path (default: output/feature_YYYYMMDD_HHmm.html)')
        ...
        if (OUTPUTFILE) {
            ...
            fs.mkdir('output', function(e) {
                if (!e || (e && e.code === 'EEXIST')) {
                    var outputFilepath = 'output/features_' + moment().format('YYYYMMDD_HHmm') + '.html';
...
async function writeOutput(html, outputfile) {
    fs.writeFileSync(outputfile, html, FILE_ENCODING);
    console.log('DONE! HTML was written to %s', outputfile);
    var pdfFIleName = outputfile.replace(".html", ".pdf");
    var wordFileName = outputfile.replace(".html", ".docx");
    ...
    fs.writeFileSync(wordFileName, converted);
    console.log("word doc generated " +wordFileName)
    // outputfile
